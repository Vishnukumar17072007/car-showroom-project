...
export default function UserDashboard() {
  ...
  const totalCarsOrdered = orders.reduce((s, o) => s + (o.items?.length || 0), 0);
  ...
  const monthlyData = useMemo(() => {
    ...
    orders.forEach((o) => {
      ...
      map[key].spending += Number(o.totalPrice || 0);
    });

    // ensure 6 months shown (fill zeros)
    const result = [];
    const now = new Date();
    for (let i = 5; i >= 0; i--) {
  ...
  const statusData = useMemo(() => {
    ...
    return Object.entries(counts)
      ...
      .map(([name, value]) => ({
        ...
        pct: totalOrders ? Math.round((value / totalOrders) * 100) : 0,
      }));
  }, [orders, totalOrders]);
                    ...
                    <tr key={order._id} onClick={() => navigate("/orders")} style={{ cursor: "pointer" }}>
                      ...
                      <td>
                        <div className="db-car-cell">
                          {car?.image && (
                            <img className="db-car-img" src={car.image} alt={car.model} />
                          )}
